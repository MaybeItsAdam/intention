...
### 7.11 The list export has no importer, and two pieces of copy imply one
...
Two places already suggest otherwise. The settings card says "you can load it
into setup and carry on where you left off" (`shared/options.html`), and
`docs/LEAVING.md` said the file "is how you pick up where you left off". The
docs half has been corrected in this pass — it now says plainly that there is
no "load this file" button and the file is a record to work from — but the
settings copy is in `shared/`, which this documentation pass does not own, and
still promises the import.

This is "we missed it" rather than "we knew and chose not to": the export
shipped as the answer to "leaving should not be punitive", and shipping only
its outbound half is defensible, but describing the outbound half as a round
trip is not. Either write the importer (it is a file input, `buildExportPayload`
read backwards, and the same `sanitizePartRule`/`normalizeLeaveDelay` validation
every other write already goes through — note `v: 1` is in the payload for
exactly this) or change one sentence in `options.html`. The second is minutes
and should not wait for the first.
